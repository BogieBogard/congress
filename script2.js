...
fs.readdir(directoryPath, function (err, files) {
    ...
    (async () => { 
        for await (const iteration of getTheLastTenSenateFiles) {
            ...
            if (senateFiles.length === 0) {
                // console.log("senate files length is 0, do nothing")
            } else {
                // Move and rename the file
                fs.rename(oldPath, newPath, function (err) {
                    if (err) console.log("Senate script error: ", err);
    ...
    (async () => { 
        for await (const iteration of getTheLastTenHouseFiles) {
            ...
            // Move and rename the file
            fs.rename(oldPath, newPath, function (err) {
                if (err) console.log("House script error: ", err);
